...
CREATE TYPE "livepoly"."game_snapshot_type" AS ENUM('start', 'turn', 'final');--> statement-breakpoint
...
ALTER TABLE "game_snapshots" ADD CONSTRAINT "game_snapshots_game_id_games_id_fk" FOREIGN KEY ("game_id") REFERENCES "livepoly"."games"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "game_snapshots" ADD CONSTRAINT "game_snapshots_room_id_rooms_id_fk" FOREIGN KEY ("room_id") REFERENCES "livepoly"."rooms"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
