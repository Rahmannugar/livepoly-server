...
Waiting rooms receive an idempotent delayed expiry job when they are created. The job runs one hour after that room's creation and cancels it only if it is still waiting. An hourly database sweep remains as reconciliation for rooms whose delayed job could not be queued or processed.
