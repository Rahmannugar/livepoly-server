...
const OUTBOX_RECOVERY_LOCK_KEY = 'worker:singleton:outbox-recovery';
const OUTBOX_RECOVERY_LOCK_TTL_SECONDS = 60;
...
export class OutboxRecoveryService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    try {
      const result = await this.cacheService.withLockOrSkip({
        key: OUTBOX_RECOVERY_LOCK_KEY,
        ttlSeconds: OUTBOX_RECOVERY_LOCK_TTL_SECONDS,
        callback: () => this.recoverAvailableEvents(),
      });
  ...
  private async recoverAvailableEvents(): Promise<void> {
    try {
